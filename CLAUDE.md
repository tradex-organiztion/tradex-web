...
5. 실제 브라우저 스크린샷으로 검증 (아래 UI 검증 워크플로우 참고)
```

#### UI 검증 워크플로우 (필수)

**UI 수정 후에는 반드시 실제 브라우저 렌더링 결과를 확인해야 합니다.**
코드만 보고 비교하지 말고, Chrome DevTools MCP를 사용하여 실제 화면을 캡처하여 Figma와 비교합니다.

```
1. dev 서버 실행 확인 (localhost:3000)
   - lsof -i :3000 으로 확인
   - 미실행 시 npm run dev 실행

2. Chrome DevTools MCP로 해당 페이지 이동
   - mcp__chrome-devtools__navigate_page로 URL 이동

3. 스크린샷 촬영
   - mcp__chrome-devtools__take_screenshot으로 현재 화면 캡처
   - fullPage: true 옵션으로 전체 페이지 캡처

4. Figma 스크린샷과 나란히 비교
   - 다운로드한 Figma 스크린샷을 Read 도구로 열기
   - 실제 렌더링 결과와 픽셀 단위로 비교

5. 차이점 발견 시 수정 후 다시 스크린샷으로 재검증
```
...
GitHub 레포지토리가 AWS Amplify에 연결되어 있어서, 브랜치에 push하면 자동으로 빌드 및 배포가 진행됩니다.
(별도 배포 명령어 없이 git push만 하면 됨)

| 브랜치 | 배포 환경 | 설명 |
|--------|----------|------|
| `develop` | 개발 서버 | push하면 개발 환경에 자동 배포 |
| `main` | 운영 서버 | push하면 프로덕션에 자동 배포 |
...
>
> **⛔ 절대 금지: Swagger에 없는 API 임의 생성 금지**
> - Swagger(OpenAPI Spec)에 **존재하지 않는 엔드포인트를 프론트엔드 코드에 임의로 만들지 말 것**
> - API 클라이언트 함수(`src/lib/api/`)는 반드시 Swagger에 실제 존재하는 엔드포인트만 구현
> - 백엔드에 아직 없는 API가 필요한 경우: 코드에 구현하지 말고 사용자에게 백엔드 API 추가 요청을 알릴 것
> - Mock 데이터가 필요하면 API 클라이언트가 아닌 컴포넌트/페이지 레벨에서 처리할 것
