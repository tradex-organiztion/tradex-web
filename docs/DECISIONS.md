...
| 4 | 음성 입력 | 🟢 확정 | Web Speech API (무료, 한국어+영어 혼용 지원) | 2026-03 |
...
| 항목 | 내용 |
|------|------|
| **결정** | Web Speech API (브라우저 내장, 무료) |
| **결정일** | 2026-03 |
| **구현 위치** | `components/layout/TradexAIPanel.tsx` → `handleMicClick` |
| **비고** | Chrome/Edge 지원. 한국어(ko-KR) + 영어 혼용 인식. 녹음 중 마이크 버튼 빨간색 애니메이션 |
...
> 현재 프론트엔드 레벨에서 미결인 사항 없음. 모든 기술적 결정 완료.

> 닉네임/프로필 이미지 수정은 UI 자체가 없음 (조회 전용으로 확정). 백엔드 API 추가 시 재검토.
